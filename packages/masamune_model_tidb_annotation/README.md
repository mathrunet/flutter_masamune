...
Use `TidbSchemaColumn("embedding", "VECTOR(3)", vectorMetric: "euclidean")` in `extraColumns` to override the type of a `ModelVectorValue` column with the same name. `vectorMetric` defaults to `cosine` and accepts `cosine` or `euclidean`; dimensions range from 1 to 16383. Distance metadata is passed to Worker queries. ANN indexes are not generated automatically.
