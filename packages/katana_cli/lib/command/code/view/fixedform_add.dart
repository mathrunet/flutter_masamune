...
///
/// 固定フォームページ（新規追加時）のテンプレートを作成します。
class CodeViewFixedFormAddCliCommand extends CliTestableCodeCommand {
  /// Create a template for the fixed form page (when adding new).
  ///
  /// 固定フォームページ（新規追加時）のテンプレートを作成します。
  const CodeViewFixedFormAddCliCommand();

  @override
  String get name => "template_fixedform_add";

  @override
  String get prefix => "templateFixedFormAdd";
  ...
  String get description =>
      "Create a template for the fixed form page (when adding new) in `$directory/(filepath).dart`. 固定フォームページ（新規追加時）のテンプレートを`$directory/(filepath).dart`に作成します。";

  @override
  String? get example => "katana code tmp fixedform_add [page_name]";
  ...
  Future<void> exec(ExecContext context) async {
    ...
    if (path.isEmpty) {
      error(
        "[path] is not specified. Please enter [path] according to the following command.\r\nkatana code view fixedform_add [path]\r\n",
      );
      return;
    }
