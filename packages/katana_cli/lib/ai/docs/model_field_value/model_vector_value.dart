...
class ModelFieldValueModelVectorValueMdCliAiCode extends ModelFieldValueCliAiCode {
  ...
  ///
  /// model_vector_value.mdの中身。
  const ModelFieldValueModelVectorValueMdCliAiCode();
...
```dart
final query = ${className}Model.collection().nearest(
  ${className}Model.vectorKey,
...
final document = ${className}Model(
  text: "サンプルテキスト",
...
final results = await ${className}Model.collection()
  .nearest(
    ${className}Model.vectorKey,
