...
/// Create an extension method for [RefHasPage] to handle Query for controllers and controller groups.
///
/// コントローラーやコントローラーグループ用のQueryを処理するための[RefHasPage]の拡張メソッドを作成します。
extension MasamuneControllerRefHasPageExtensions on RefHasPage {
  /// TController] is obtained by passing [ControllerQueryBase] for the form generated by the builder.
  ///
  /// The state is always saved in page scope.
  ///
  /// ビルダーによりコード生成されたフォーム用の[ControllerQueryBase]を渡すことにより状態を保持された[TController]を取得します。
  ///
  ...
  /// ```
  TController form<TController extends Listenable>(
    ControllerQueryBase<TController> query,
  ) {
