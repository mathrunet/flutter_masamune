...
void main() {
  testWidgets("MarkdownField.blockProperty.quote.insert", (tester) async {
    ...
    controller.insertBlock(const QuoteAddMarkdownBlockTools());
  ...
  testWidgets("MarkdownField.blockProperty.quote.exchange",
      (tester) async {
    final context = await buildMarkdownField(tester);
    final controller = context.controller;
    final input = context.input;
    ...
    controller.exchangeBlock(const QuoteExchangeMarkdownBlockTools());
    expect(controller.selection.baseOffset, 5);
    expect(controller.plainText, "aaa\nbbb");
    expect(controller.rawText, "aaa\nbbb");
    expect(
  ...
  testWidgets("MarkdownField.blockProperty.quote.newLine",
      (tester) async {
    final context = await buildMarkdownField(tester);
    final controller = context.controller;
    final input = context.input;
  ...
  testWidgets("MarkdownField.blockProperty.quote.undoRedo",
      (tester) async {
    final context = await buildMarkdownField(tester);
    final controller = context.controller;
    final input = context.input;
