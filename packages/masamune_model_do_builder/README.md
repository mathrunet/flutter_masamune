<p align="center">
  ...
  <h1 align="center">Masamune Model Durable Objects Builder</h1>
</p>
...
Generate Durable Objects schema fragments from Masamune models annotated with
[`@DurableObjectSchema`](../masamune_model_do_annotation/README.md).

```yaml
dependencies:
  masamune_model_do_annotation: ^3.1.0

dev_dependencies:
  masamune_model_do_builder: ^3.1.0
```
...
The builder writes a `.do_schema` fragment for each input file. Katana CLI
aggregates all fragments into `do/schema/schema.json` by default, preserving
...
types are rejected. Vectorize configuration and `ModelVectorValue` columns are not generated by this
builder. See the annotation package for the current vector schema limitations.
