...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void initState() {
    ...
    collection = FirestoreDynamicCollection(CollectionModelQuery(
      "test",
      adapter: adapter,
      key: "@search",
      search: "46",
      ...
      child: Scaffold(
        ...
        body: Column(
          ...
          children: collection.mapListenable((item) {
            return ListTile(
                title: Text(item.value.get("name", "")),
                onTap: () {
                  item.value = {"name": DateTime.now().toDateTimeID()};
                  item.save();
                },
                trailing: IconButton(
                  onPressed: () {
        ...
        floatingActionButton: FloatingActionButton(
          onPressed: () {
            ...
            doc.value = {"name": DateTime.now().toDateTimeID()};
            doc.save();
          },
