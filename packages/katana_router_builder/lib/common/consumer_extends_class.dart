...
List<Class> consumerExtendsClass(ClassModel model, PathModel path) {
  return [
    Class(
      (c) => c
        ..name = "_${model.name}"
    ...
    Class(
      (c) => c
        ..name = "_\$${model.name}"
        ..abstract = true
        ..extend = Reference("PageConsumerWidgetBuilder<\$${model.name}>")
        ..annotations = ListBuilder([const Reference("immutable")])
    ...
    Class(
      (c) => c
        ...
        ..extend = Reference(
          "PageConsumerWidgetBuilderState<\$${model.name}, _${model.name}>",
        ),
    ),
  ];
}
